...
from wilor.configs import dataset_eval_config
from wilor.datasets import create_dataset
...
from wilor.utils import recursive_to, Evaluator
...
def main():
    ...
    tmu.patch.timm(model.backbone, trace_source=True)
    model.backbone.r = args.r
    print("\n\nModel model.backbone.r:", model.backbone.r)
...
def run_eval(model, model_cfg, dataset_cfg, device, args):
    ...
    image_per_second = total_count / total_time
    print(f"Total sample: {total_count}")
    print(f"Image per second: {image_per_second} img/s")
    ...
    if preds is not None:
        pred_name = args.dataset.lower() + "_r" + str(args.r) + ".json"
        results_json = os.path.join(args.results_folder, pred_name)
