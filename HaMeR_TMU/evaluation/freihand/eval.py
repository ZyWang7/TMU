...
def align_w_scale(mtx1, mtx2, return_trafo=False):
    ...
    if np.any(np.isnan(mtx1)) or np.any(np.isnan(mtx2)):
        raise ValueError("NaNs found in input matrices for alignment.")
    if np.allclose(mtx1, 0) or np.allclose(mtx2, 0):
        raise ValueError("Zero vectors detected in alignment inputs.")
...
def createHTML(outputDir, curve_list):
    ...
    for item in curve_list:
        ...
        img_tag1 = "data:image/png;base64," + data_uri1
    ...
    for i, (text, img_embed) in enumerate(curve_data_list):
        ...
        <h2>%s</h2>
        <p>
        <img border="0" src="%s" alt="FROC" width="576pt" height="432pt">
        </p>
        <p>Raw curve data:</p>
...
if __name__ == '__main__':
    ...
    parser.add_argument('--r', type=int, default=0, help='Number of tokens merged per layer.')
    args = parser.parse_args()

    gt_path = "evaluation/freihand/gt"
    pred_path = "results"
    output_dir = "evaluation/freihand/eval_results/r" + str(args.r)
    pred_file_name = "freihand-val_r" + str(args.r) + ".json"
    set_name='evaluation'
    os.makedirs(output_dir, exist_ok=True)

    main(gt_path, pred_path, output_dir, pred_file_name, set_name)
