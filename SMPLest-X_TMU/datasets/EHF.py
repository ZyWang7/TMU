...
from human_models.human_models import SMPLX as smpl_x
from utils.data_utils import load_img, process_bbox, augmentation, load_ply
...
class EHF(torch.utils.data.Dataset):
    def __init__(self, transform, data_split, cfg):
        ...
        self.data_path = cfg.data.data_dir
    ...
    def load_data(self):
        datalist = []
        db = COCO(osp.join(self.data_path, 'EHF/EHF.json'))
        for aid in db.anns.keys():
            ...
            bbox = ann['body_bbox']
            bbox = process_bbox(bbox, img['width'], img['height'], self.cfg.model.input_img_shape, ratio=getattr(self.cfg.data, 'bbox_ratio', 1.25))
            if bbox is None:
                continue

            lhand_bbox = np.array(ann['lefthand_bbox']).reshape(4)
            if hasattr(self.cfg.data, 'bbox_ratio'):
                lhand_bbox = process_bbox(lhand_bbox, img['width'],  img['height'], self.cfg.model.input_img_shape, ratio=self.cfg.data.bbox_ratio)
            lhand_bbox[2:] += lhand_bbox[:2]  # xywh -> xyxy

            rhand_bbox = np.array(ann['righthand_bbox']).reshape(4)
            if hasattr(self.cfg.data, 'bbox_ratio'):
                rhand_bbox = process_bbox(rhand_bbox, img['width'], img['height'], self.cfg.model.input_img_shape, ratio=self.cfg.data.bbox_ratio)
            rhand_bbox[2:] += rhand_bbox[:2]  # xywh -> xyxy

            face_bbox = np.array(ann['face_bbox']).reshape(4)
            if hasattr(self.cfg.data, 'bbox_ratio'):
                face_bbox = process_bbox(face_bbox, img['width'], img['height'], self.cfg.model.input_img_shape, ratio=self.cfg.data.bbox_ratio)
    ...
    def process_hand_face_bbox(self, bbox, do_flip, img_shape, img2bb_trans):
        if bbox is None:
            ...
            bbox[:, 0] = bbox[:, 0] / self.cfg.model.input_img_shape[1] * self.cfg.model.output_hm_shape[1]
            bbox[:, 1] = bbox[:, 1] / self.cfg.model.input_img_shape[0] * self.cfg.model.output_hm_shape[0]
    ...
    def __getitem__(self, idx):
        ...
        # affine transform
        img, img2bb_trans, bb2img_trans, rot, do_flip = augmentation(self.cfg.data.no_aug, img, bbox, self.data_split, self.cfg.model.input_img_shape)
    ...
    def evaluate(self, outs, cur_sample_idx, smpl_x):
