...
from models.module import Attention_ViT, Block, ViT
...
class TMUAttention(Attention_ViT):
    """
    Modifications:
    ...
    def forward(
        ...
        # The metric for merging is the mean of K over the heads
        k_mean = k.mean(dim=1)

        # Create the proportional attention mask if token sizes are provided
        attn_mask = None
        if size is not None:
        ...
        # Return the features and the merging metric
        return x, k_mean
...
def make_TMU_class(transformer_class):
    class TMUVisionTransformer(transformer_class):
        ...
        def _prune_forward(self, x):
            ...
            print(f"Number of tokens left: {x.shape[1]-80}")
            ...
            unmerged_features = self.unmerge_tmu_output_gather(x, source_matrix)
            # print("unmerged_features shape:", unmerged_features.shape)

            # x: [B, 272, 1280]
            x = self.last_norm(unmerged_features)
            
            task_tokens = x[:, :self.task_tokens_num]   # [B, 80, 1280]
            xp = x[:, self.task_tokens_num:]    # [B, 192, 1280]

            print(f"Number of image tokens: {xp.shape[1]}")
        ...
        def unmerge_tmu_output_gather(self, x: torch.Tensor, source: torch.Tensor) -> torch.Tensor:
            """
            Unmerges the output of a tmu-applied layer using an explicit gather operation.
            
            Args:
                x (torch.Tensor): The feature tensor of merged tokens. Shape: (B, Merged_N, F).
                source (torch.Tensor): The mapping matrix. Shape: (B, Merged_N, Original_N).
                                    A '1' at source[b, j, i] means original token 'i'
                                    was merged into token 'j'.
            """
            # 1. Find the mapping indices.
            # We want to know, for each original token, which merged token it came from.
            # `source` has shape (B, Merged_N, Original_N). `argmax` along dim=1
            # finds the index 'j' (from Merged_N) for each 'i' (in Original_N).
            # The result `indices` will have the shape (B, Original_N).
            indices = source.argmax(dim=1)

            # 2. Prepare the index tensor for gathering.
            # `torch.gather` requires the index tensor to have the same number of dimensions
            # as the input tensor 'x'. We need to add a feature dimension and expand it.
            # Shape: (B, Original_N) -> (B, Original_N, 1) -> (B, Original_N, F)
            f_dim = x.shape[-1]
            indices = indices.unsqueeze(-1).expand(-1, -1, f_dim)

            # 3. Perform the gather operation.
            # This operation effectively says: "For each of the Original_N positions,
            # go to the 'x' tensor and pull the feature vector specified by our 'indices' tensor."
            # It gathers along dim=1 (the token dimension).
            unmerged_x = torch.gather(x, dim=1, index=indices)
...
def apply_patch(
    ...
    for module in model.modules():
        ...
        elif isinstance(module, Attention_ViT):
