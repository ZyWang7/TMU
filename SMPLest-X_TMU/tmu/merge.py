...
def bipartite_soft_matching(
    ...
    # Separate image tokens and smpl tokens
    img_tokens = metric[:, :-80, :]  # All tokens except last 80
    smpl_tokens = metric[:, -80:, :]  # Last 80 tokens are task tokens
    ...
    def merge(x: torch.Tensor, mode="mean") -> torch.Tensor:
        # Separate image tokens and task tokens
        img_x = x[:, :-80, :]
        smpl_x = x[:, -80:, :]
        ...
        # Concatenate merged image tokens with unchanged task tokens
        return torch.cat([merged_img, smpl_x], dim=1)

    def unmerge(x: torch.Tensor) -> torch.Tensor:
        # Separate merged image tokens and task tokens
        merged_img_len = x.shape[1] - 80
        merged_img = x[:, :merged_img_len, :]
        smpl_x = x[:, merged_img_len:, :]
        ...
        # Concatenate with task tokens
        return torch.cat([out_img, smpl_x], dim=1)
